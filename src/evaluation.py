...
parser.add_argument("--prompt", type = str, default = "base_prompt", help = "base_prompt/coding_concepts/chain_of_thought/one_shot")
...
if(args.metric == "classification"):
    ...
    op_dict = {i: joint_accuracy[i] for i in range(len(joint_accuracy))}
    print(op_dict)
...
# To calculate diffbleu(Applicable for all splits)
    ...
    with jsonlines.open(generations_path) as reader:
        ...
        for generation in reader:
            ...
            for l in range(args.num_samples):
                ...
                outputs.append([generated_answers[1], generation['target_code']])
    ...
    score_dict = {i: overall_score[k][i] for i in range(len(overall_score[k]))}
